...
v_values = [2.84e-6, 1.84e-6, 0.84e-6];

% Initialize arrays to store the results for each permeability value
results = cell(length(v_values), 1);

for v_0 = 1:length(v_values)
    Vmax = v_values(v_0); % 
% Set the boundary value and initialize concnetration for Rm
Cs(1, 1) = Cm(end, numt);
...
results{v_0} = Cs(end, 1:numt);
end



% Define the base directory where you want to create subfolders
baseDirectory = '/Users/walton_user/Documents/MATLAB/Scaffold_Uptake_Diffusion_Porosity_updated_12JUNE'; % Update this path

% Create subfolders for each permeability value
v_values = [2.84e-6, 1.84e-6, 0.84e-6]; % Permeability values
for v_0 = 1:length(v_values)
    folderName = sprintf('uptake_rate_%d', v_0);
    ...
    for v_0 = 1:length(v_values)
        ...
        header = {'Time', ['Concentration_Uptake_Rate_', num2str(v_0)]};
        ...
        filename = sprintf('concentration_uptake_rate_seed%d.csv', seed);
