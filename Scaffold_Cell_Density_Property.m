...
N_values = [4000, 3000, 2000]; % Permeability values at the interface between membrane and scaffold (Rm)
% Initialize arrays to store the results for each permeability value
results = cell(length(N_values), 1);

for N_0 = 1:length(N_values)
    N0 = N_values(N_0); % Set the current permeability value
% Set the boundary value and initialize concnetration for Rm
Cs(1, 1) = Cm(end, numt);
...
results{N_0} = Cs(end, 1:numt);
end


% Define the base directory where you want to create subfolders
baseDirectory = '/Users/walton_user/Documents/MATLAB/Scaffold_Cell_Density_12_june'; % Update this path

% Create subfolders for each permeability value
N_values = [4000, 3000, 2000];  % Permeability values
for N_0 = 1:length(N_values)
    folderName = sprintf('Cell_Density_%d', N_0);
    ...
    for N_0 = 1:length(N_values)
        ...
        header = {'Time', ['Concentration_Cell_Density_', num2str(N_0)]};
        ...
        filename = sprintf('concentration_cell_density_seed%d.csv', seed);
