...
source_term = zeros(num_points_Re, 1);
   % Calculate the number of molecules to be released in this time step using a Poisson distribution
    ...
    source_term = random_release_count * exp(-(r_Re - mu).^2 / (2 * sigma^2)) / sqrt(2 * pi * sigma^2);
      C(1,1) = source_term(1);
...
P_values = [0.001, 0.01, 0.1]; % Permeability values at the interface between membrane and scaffold (Rm)
% Initialize arrays to store the results for each permeability value
results_cm = cell(length(P_values), 1);
...
baseDirectory = '/Users/walton_user/Documents/MATLAB/Membrane_Permeability_UR'; % Update this path

% Create subfolders for each permeability value

P_values = [0.001, 0.01, 0.1];  % Permeability values
for p = 1:length(P_values)
